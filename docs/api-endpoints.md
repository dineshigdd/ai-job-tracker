...
> Written against the live route table (`app.openapi()`). Regenerate after adding or
> renaming routes rather than editing entries individually.

## 1. Overview

- **Base URL (local):** `http://localhost:8000` — there is no `/api` or `/v1` prefix.
  The router prefixes (`/jobs`, `/users`, `/resumes`, `/dashboard`) are the whole path.
- **Authentication:** `POST /login` sets an **HttpOnly cookie** named `access_token`.
  Browsers send it automatically; JavaScript cannot read it. An
  `Authorization: Bearer <token>` header is also accepted as a fallback, which is what
  the Swagger UI **Authorize** button uses.
